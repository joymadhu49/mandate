...
test('provider redirects are rejected without following them or forwarding credentials', async t => {
  ...
  const priorRelayKey = process.env.RELAY_API_KEY;
  process.env.RELAY_API_KEY = 'test-only-relay-key';
  t.after(() => { if (priorRelayKey === undefined) delete process.env.RELAY_API_KEY; else process.env.RELAY_API_KEY = priorRelayKey; });
  const { quoteSwap } = await import('./lifi.js');
  const { quoteSwap: relayQuoteSwap } = await import('./relay.js');
  ...
  const fetchMock = t.mock.method(globalThis,'fetch',async (_input: unknown, init?: RequestInit) => {
    ...
    if (String(_input).startsWith('https://api.relay.link/')) assert.equal(new Headers(init?.headers).get('x-api-key'), 'test-only-relay-key');
  ...
  await assert.rejects(relayQuoteSwap({from:'0x0000000000000000000000000000000000000001',fromToken:'0x0000000000000000000000000000000000000000',toToken:'0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913',fromAmount:1n}), /Swap quote unavailable/); // gitleaks:allow -- public Base USDC contract address, not an API token
